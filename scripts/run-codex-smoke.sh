#!/usr/bin/env bash
set -euo pipefail
codex_version=""; packy_ref=""; evidence_dir=""
while (($#)); do case "$1" in --codex-version) codex_version="${2:-}";shift 2;;--packy-ref) packy_ref="${2:-}";shift 2;;--evidence-dir) evidence_dir="${2:-}";shift 2;;*) echo "unknown argument: $1" >&2;exit 2;;esac;done
...
env -i PATH="$restricted_path" TMPDIR="${TMPDIR:-/tmp}" "$build/codexsmoke" --codex "$build/npm/node_modules/.bin/codex" --search-path "$restricted_path" --codex-version "$resolved" --codex-integrity "$integrity" --packy-ref "$packy_ref" --packy-sha "$sha" --evidence "$evidence_dir/evidence.json"
