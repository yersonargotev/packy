...
`packy pack install <pack>` changes project intent and converges its lock and projections. `packy pack install` without a pack reconciles the complete project against the human-edited manifest: it validates exact versions, regenerates a stale lock, previews the complete change, and applies one recoverable transaction. It may restore a missing exact owned projection, but blocks on changed or foreign content and never advances a version implicitly.
