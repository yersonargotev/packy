...
func (a *SurfaceAdapter) inspectDesired(_ context.Context, pack capabilitypack.Pack, resolutions []capabilitypack.ExecutableResolution) (capabilitypack.SurfaceInspection, error) {
	...
	for _, resource := range pack.Resources {
		...
		if hasPrimaryPrompt {
			...
			content, err := os.ReadFile(filepath.Join(catalogRoot(resource, a.bundleRoot), filepath.Clean(data.Source)))
			if err != nil {
		...
		case "agent":
			...
			prompt, err := os.ReadFile(filepath.Join(catalogRoot(resource, a.bundleRoot), filepath.Clean(resource.Source)))
			if err != nil {
