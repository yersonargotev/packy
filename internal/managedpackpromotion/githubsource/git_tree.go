...
// It reads the recursive Git tree and every blob at the exact commit, verifies
// Git object identities, and never requests a generated archive.
func (client *Client) WithGitTreeSnapshot(ctx context.Context, candidate Candidate, temporaryRoot string, visit func(string) error) (result error) {
	if visit == nil {
	...
	if err := validateRecursiveGitTree(tree, candidate.Tree, client.treeLimits); err != nil {
		return err
	}
...
func validateGitTreeCandidate(candidate Candidate) error {
...
func (client *Client) readRecursiveGitTree(ctx context.Context, candidate Candidate) (recursiveGitTree, error) {
	...
	data, err := io.ReadAll(io.LimitReader(contextReader{ctx: ctx, reader: response.Body}, maxGitTreeResponseBytes+1))
	if err != nil {
		return recursiveGitTree{}, err
	}
	if len(data) > maxGitTreeResponseBytes {
		return recursiveGitTree{}, fmt.Errorf("recursive Git tree response exceeds %d bytes", maxGitTreeResponseBytes)
	}
...
func validateRecursiveGitTree(tree recursiveGitTree, expectedRoot string, limits gitTreeLimits) error {
	...
	for _, entry := range tree.Entries {
		if !safeGitTreePath(entry.Path) || seen[entry.Path] {
...
func (client *Client) materializeVerifiedGitTree(ctx context.Context, candidate Candidate, tree recursiveGitTree, snapshot string) error {
...
func (client *Client) readExactCommitBlob(ctx context.Context, candidate Candidate, entry gitTreeEntry) ([]byte, error) {
