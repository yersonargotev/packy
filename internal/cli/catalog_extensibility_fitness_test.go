...
func TestRenamedCatalogPackPreservesReadinessAcrossSurfacesScopesAndPresentations(t *testing.T) {
	...
	for _, surface := range []string{"codex", "opencode", "claude"} {
		...
		for _, fact := range []string{`"configured":"true"`, `"usable":"unknown"`, `"surface-authorization"`, `"runtime-usability"`} {
		...
		for _, fact := range []string{`"configured":"true"`, `"usable":"unknown"`, `"type":"surface-authorization"`, `"type":"runtime-usability"`} {
	...
	for _, surface := range []string{"claude", "codex", "opencode"} {
		...
		if err != nil || !strings.Contains(status, `"configured":"true"`) || !strings.Contains(status, `"usable":"unknown"`) {
	...
	for _, surface := range []string{"claude", "codex", "opencode"} {
		...
		if index < 0 || pack.SurfaceStatuses[index].Configured != "true" || pack.SurfaceStatuses[index].Authorized == "" || pack.SurfaceStatuses[index].Usable != "unknown" || len(pack.SurfaceStatuses[index].Conditions) < 3 || len(pack.SurfaceStatuses[index].PendingActions) != wantPending {
