...
func TestIssue518ActivationPublishesMinimalInstalledPackReceipt(t *testing.T) {
	...
	if receipt.Pack.ID != "matty" || receipt.Pack.Version != "1.0.0" || receipt.Surface != "codex" || len(receipt.Resources) == 0 || len(receipt.Projections) == 0 {
...
func TestIssue518StatusReportsCurrentReceiptWithoutHistoricalAttempts(t *testing.T) {
	...
	for _, fact := range []string{"matty 1.0.0 on codex", "Resources:", "Readiness:", "Receipt ownership:", "Drift:"} {
