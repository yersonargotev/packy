...
func TestClassicLifecycleAuthorityIsDeleted(t *testing.T) {
	for _, path := range []string{"classic_lifecycle_output.go", "../claudecode/classic.go"} {
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			t.Fatalf("classic lifecycle authority %s still exists: %v", path, err)
		}
	}
	for _, pattern := range []string{"../corelifecycle/*.go", "../ownedcontainer/*.go"} {
		matches, err := filepath.Glob(pattern)
		if err != nil || len(matches) != 0 {
			t.Fatalf("classic lifecycle authority %s still has Go files: %v, %v", pattern, matches, err)
		}
	}
	...
	for _, source := range cliGoSources(t) {
		...
		for _, forbidden := range []string{"corelifecycle", "executeClassicLifecycle", "newInstallCommand", "newUpdateCommand", "newUninstallCommand"} {
			if strings.Contains(source.text, forbidden) {
				t.Fatalf("%s retained classic lifecycle authority %q", source.name, forbidden)
			}
		}
	}
}
