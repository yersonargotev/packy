...
func main() {
	...
	provenance, err := loadProvenance(*root, details)
	if err != nil {
		fail("load Managed Pack provenance: %v", err)
	}
	expected := renderDocumentation(details, provenance)
...
type packProvenance struct {
	Project string
	Record  managedpack.AdmissionRecord
}

func loadProvenance(root string, details []capabilitypack.CatalogDetail) (map[string]packProvenance, error) {
	...
	projects := make(map[string]string, len(registry.Packs))
	for _, registration := range registry.Packs {
		projects[registration.PackID] = registration.Project
	}
	result := make(map[string]packProvenance, len(details))
	for _, detail := range details {
		project, ok := projects[detail.Pack.ID]
		if !ok {
			return nil, fmt.Errorf("Pack %q is not registered", detail.Pack.ID)
		}
		record, err := managedpack.LoadAdmissionRecord(filepath.Join(root, "managed-packs", "admissions", detail.Pack.ID, detail.Pack.Version+".json"))
		if err != nil {
			return nil, err
		}
		result[detail.Pack.ID] = packProvenance{Project: project, Record: record}
	}
	return result, nil
}

func renderDocumentation(details []capabilitypack.CatalogDetail, provenance map[string]packProvenance) map[string][]byte {
	files := map[string][]byte{"index.md": renderIndex(details)}
	for _, detail := range details {
		files[detail.Pack.ID+".md"] = renderPack(detail, provenance[detail.Pack.ID])
	}
	return files
}
...
func renderPack(detail capabilitypack.CatalogDetail, provenance packProvenance) []byte {
	pack := detail.Pack
	var out bytes.Buffer
	out.WriteString(generatedNotice)
	fmt.Fprintf(&out, "\n# %s Pack\n\n", title(pack.ID))
	...
	out.WriteString("\n## Provenance\n\n")
	fmt.Fprintf(&out, "- Managed Pack Project: [`%s`](https://github.com/%s)\n", provenance.Project, provenance.Project)
	fmt.Fprintf(&out, "- Immutable release: [`%s`](https://github.com/%s/releases/tag/%s)\n", provenance.Record.Tag, provenance.Project, provenance.Record.Tag)
	fmt.Fprintf(&out, "- Pack Admission Record: [`managed-packs/admissions/%s/%s.json`](../../managed-packs/admissions/%s/%s.json)\n", pack.ID, pack.Version, pack.ID, pack.Version)
