...
func gateFailure(code LocalGateErrorCode, format string, args ...any) (LocalGateReport, error) {
	return LocalGateReport{}, &LocalGateError{Code: code, Detail: fmt.Sprintf(format, args...)}
}
...
func EvaluateLocalGate(bundle Bundle, observation LocalGateObservation) (LocalGateReport, error) {
	if observation.Repository != bundle.Repository || observation.Issue != bundle.Issue || observation.Spec != bundle.Spec {
		return gateFailure(LocalGateForeignEvidence, "observation identities do not belong to the bundle")
	}
	if !observation.IssueEligible || !observation.SpecEligible {
		return gateFailure(LocalGateTrackerAuthorityChanged, "issue or specification is no longer eligible")
	}
	if observation.IssueSHA256 != bundle.Authority.IssueSHA256 || observation.SpecSHA256 != bundle.Authority.SpecSHA256 {
		return gateFailure(LocalGateTrackerAuthorityChanged, "qualified tracker authority has changed")
	}
	if err := Validate(bundle); err != nil {
		return gateFailure(LocalGateQualificationInvalid, "%v", err)
	}
	var unproved []string
	...
	if len(unproved) > 0 {
		return gateFailure(LocalGateAcceptanceUnproved, "acceptance rows are not proved: %s", strings.Join(unproved, ", "))
	}
	if len(bundle.Iterations) == 0 {
		return gateFailure(LocalGateUnrecordedDelta, "at least one contiguous iteration is required")
	}
	if strings.TrimSpace(observation.ExpectedBranch) == "" || observation.CurrentBranch != observation.ExpectedBranch {
		return gateFailure(LocalGateWrongBranch, "expected branch %q, observed %q", observation.ExpectedBranch, observation.CurrentBranch)
	}
	if !observation.Validation.WorkspaceClean {
		return gateFailure(LocalGateDirtyWorkspace, "workspace is not clean")
	}
	last := bundle.Iterations[len(bundle.Iterations)-1]
	if last.HeadSHA != observation.HeadSHA {
		return gateFailure(LocalGateUnrecordedDelta, "final iteration head %s does not equal current head %s", last.HeadSHA, observation.HeadSHA)
	}
	status, err := QueryReviews(bundle, observation.HeadSHA, observation.OrderedCommits)
	if err != nil {
		return gateFailure(LocalGateUnrecordedDelta, "%v", err)
	}
	if len(status.UnreviewedDeltas) > 0 {
		return gateFailure(LocalGateReviewGap, "iterations lack both review axes: %s", strings.Join(status.UnreviewedDeltas, ", "))
	}
	if len(status.UncoveredCommits) > 0 {
		return gateFailure(LocalGateUnrecordedDelta, "commits lack recorded iteration coverage: %s", strings.Join(status.UncoveredCommits, ", "))
	}
	if len(status.UnresolvedFindings) > 0 {
		return gateFailure(LocalGateUnresolvedFindings, "findings remain unresolved: %s", strings.Join(status.UnresolvedFindings, ", "))
	}
	if observation.Validation.CommitSHA != observation.HeadSHA || observation.Validation.TreeSHA != observation.TreeSHA {
		return gateFailure(LocalGateStaleValidation, "current validation does not seal the observed head and tree")
	}
	receipt, err := ReusableExhaustiveValidation(bundle, observation.Validation, observation.ObservedAt)
	if err != nil {
		return gateFailure(LocalGateStaleValidation, "%v", err)
	}
	digest, err := Digest(bundle)
	if err != nil {
		return gateFailure(LocalGateQualificationInvalid, "%v", err)
	}
	return LocalGateReport{Repository: bundle.Repository, Issue: bundle.Issue, Spec: bundle.Spec, IssueNumber: bundle.Issue.Number, Branch: observation.CurrentBranch, StartingBaseSHA: bundle.StartingBaseSHA, HeadSHA: observation.HeadSHA, TreeSHA: observation.TreeSHA, Owned: len(bundle.Scope.OwnedNow), Deferred: len(bundle.Scope.Deferred), Forbidden: len(bundle.Scope.Forbidden), Prerequisites: len(bundle.Scope.Prerequisites), AcceptanceProved: len(bundle.AcceptanceMatrix), Iterations: len(bundle.Iterations), Reviews: len(bundle.ReviewReceipts), Adjudications: len(bundle.Adjudications), ReviewedCommits: len(observation.OrderedCommits) - 1, ValidationCompletedAt: receipt.CompletedAt, BundleSHA256: digest}, nil
}

// RenderLocalGateReport renders a deterministic, single-observation success report.
func RenderLocalGateReport(report LocalGateReport) string {
	return fmt.Sprintf("LOCAL delivery gate: PASS\nRepository: %s/%s (%s)\nIssue: #%d (%s)\nSpec: #%d (%s)\nBranch: %s\nStarting base: %s\nHead: %s\nTree: %s\nScope: owned=%d deferred=%d forbidden=%d prerequisites=%d\nAcceptance proved: %d\nIterations: %d\nReview receipts: %d\nAdjudications: %d\nReviewed commits: %d\nExhaustive validation completed: %s\nBundle SHA-256: %s\n", report.Repository.Owner, report.Repository.Name, report.Repository.NodeID, report.Issue.Number, report.Issue.NodeID, report.Spec.Number, report.Spec.NodeID, report.Branch, report.StartingBaseSHA, report.HeadSHA, report.TreeSHA, report.Owned, report.Deferred, report.Forbidden, report.Prerequisites, report.AcceptanceProved, report.Iterations, report.Reviews, report.Adjudications, report.ReviewedCommits, report.ValidationCompletedAt, report.BundleSHA256)
}
