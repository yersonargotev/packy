...
var durableDiscover = DiscoverForDurableIntents
var packageAliasedDurableCatalog, _ = durableDiscover(ctx, filepath.Join("..", "..", "bundle"))
var catalogBuilder = catalogFor
var packageAliasedHelperCatalog = catalogBuilder(filepath.Join("..", "..", "bundle"))
...
func TestAliasedDiscoverLive(t *testing.T) { _, _ = durableDiscover(ctx, filepath.Join(repositoryRoot(), "bundle")) }
func TestAliasedDiscoverTemporary(t *testing.T) { _, _ = durableDiscover(ctx, filepath.Join(t.TempDir(), "bundle")) }
...
func TestPackageAliasedDurableCatalogBinding(t *testing.T) { _ = packageAliasedDurableCatalog.List() }
func TestPackageAliasedHelperCatalogBinding(t *testing.T) { _ = packageAliasedHelperCatalog.List() }
func TestAliasedCatalogBuilderTemporary(t *testing.T) { _ = catalogBuilder(filepath.Join(t.TempDir(), "bundle")) }
...
func TestFacadePreviewProjectUpdateLive(t *testing.T) { liveFacade().PreviewProjectUpdate(ctx, ProjectUpdateRequest{}, nil) }
func TestFacadePreviewLive(t *testing.T) { liveFacade().Preview(ctx, ActivationRequest{}) }
func TestFacadePreviewUpdateLive(t *testing.T) { liveFacade().PreviewUpdate(ctx, UpdateRequest{}) }
...
`),
	...
	for _, want := range []string{
		...
		"sample/scenarios_test.go:TestAliasedDiscoverLive",
		...
		"sample/scenarios_test.go:TestPackageAliasedDurableCatalogBinding",
		"sample/scenarios_test.go:TestPackageAliasedHelperCatalogBinding",
	...
	for test, detail := range map[string]string{
		...
		"sample/scenarios_test.go:TestPackageAliasedDurableCatalogBinding":   "binds the checked-in bundle",
		"sample/scenarios_test.go:TestPackageAliasedHelperCatalogBinding":    "binds the checked-in bundle",
	...
	for _, unwanted := range []string{
		...
		"sample/scenarios_test.go:TestAliasedDiscoverTemporary",
...
type ProjectActivationCategory int
...
func (Catalog) List() []Pack { return nil }
func NewFacade(catalog Catalog) Facade { return Facade{catalog: catalog} }
func (Facade) Status(context.Context, StatusRequest) {}
func (Facade) ActiveStatus(context.Context) {}
func (Facade) PreviewProjectInstall(context.Context, ProjectInstallRequest, SurfaceAdapter) {}
func (Facade) PreviewProjectUpdate(context.Context, ProjectUpdateRequest, SurfaceAdapter) {}
func (Facade) Preview(context.Context, ActivationRequest) {}
func (Facade) PreviewUpdate(context.Context, UpdateRequest) {}
func (Facade) PreviewDeactivate(context.Context, DeactivationRequest) {}
func (Facade) Apply(context.Context, ApplyRequest) {}
func (Facade) Show(context.Context, string) {}
func (Facade) CheckProjectInstallFreshness(context.Context, JSONProjectInstallPreview, SurfaceAdapter) {}
func (Facade) ApplyProjectInstall(context.Context, ProjectInstallApplyRequest) {}
func (Facade) PreviewControlledCheck(context.Context, ControlledCheckRequest) {}
func (Facade) RecordControlledCheck(context.Context, ControlledCheckPreview, ReadinessValue) {}
func (Facade) InspectProjectStatus(context.Context, ProjectStatusRequest) {}
func (Facade) PreviewProjectActivation(context.Context, ProjectActivationRequest) {}
func (Facade) ApplyProjectActivation(context.Context, ProjectActivationApplyRequest) {}
func (Facade) Approve(ReconciliationPlan, ConsentKind) ApprovalReceipt { return ApprovalReceipt{} }
func (Facade) ApproveProjectActivation(JSONProjectActivationPreview, ProjectActivationCategory) ProjectActivationApproval { return ProjectActivationApproval{} }
...
func (unrelatedView) PreviewControlledCheck(context.Context, ControlledCheckRequest) {}
`
...
func (context *realCatalogSSAContext) resolvedCallees(call *ssa.CallCommon) []*ssa.Function {
	seen := map[*ssa.Function]bool{}
	var result []*ssa.Function
	for _, callee := range possibleSSAFunctions(call.Value, context.analyzer, map[ssa.Value]bool{}) {
...
func (analyzer *realCatalogSSAAnalyzer) callFindings(context *realCatalogSSAContext, caller *ssa.Function, call *ssa.CallCommon) []string {
	var details []string
	for _, callee := range context.resolvedCallees(call) {
		details = append(details, analyzer.callFindingsForCallee(context, caller, call, callee)...)
	}
	return uniqueStrings(details)
}

func (analyzer *realCatalogSSAAnalyzer) callFindingsForCallee(context *realCatalogSSAContext, caller *ssa.Function, call *ssa.CallCommon, callee *ssa.Function) []string {
	var direct []string
...
func (analyzer *realCatalogSSAAnalyzer) isFacadeObservation(function *ssa.Function) bool {
	if function == nil || function.Signature.Recv() == nil {
		return false
	}
	named := namedType(function.Signature.Recv().Type())
	if named == nil || named.Obj().Name() != "Facade" || named.Obj().Pkg() == nil || !analyzer.fixture && !strings.HasSuffix(named.Obj().Pkg().Path(), "/internal/capabilitypack") {
		return false
	}
	switch function.Name() {
	case "Status", "ActiveStatus", "PreviewProjectInstall", "PreviewProjectUpdate", "Preview", "PreviewUpdate", "PreviewDeactivate", "Apply", "Show",
...
func (analyzer *realCatalogSSAAnalyzer) manifestValueFinding(context *realCatalogSSAContext, caller *ssa.Function, value ssa.Value, seen map[ssa.Value]bool) string {
	...
	case *ssa.Call:
		...
		var detail string
		context.forEachCallReturn(call, 0, func(resolved *ssa.Function, returned ssa.Value) {
			if detail == "" {
				detail = analyzer.manifestValueFinding(context, resolved, returned, seen)
			}
		})
		return detail
	case *ssa.Extract:
		if call, ok := value.Tuple.(*ssa.Call); ok {
			var detail string
			context.forEachCallReturn(call.Common(), value.Index, func(resolved *ssa.Function, returned ssa.Value) {
				if detail == "" {
					detail = analyzer.manifestValueFinding(context, resolved, returned, seen)
				}
			})
...
func (context *realCatalogSSAContext) catalogUsesLiveBundle(value ssa.Value, seen map[ssa.Value]bool) bool {
	...
	if call, ok := value.(*ssa.Call); ok {
		...
		context.forEachCallReturn(call.Common(), 0, func(_ *ssa.Function, returned ssa.Value) {
...
func (context *realCatalogSSAContext) facadeUsesLiveCatalog(value ssa.Value, seen map[ssa.Value]bool) bool {
	...
	case *ssa.Call:
		found := false
		context.forEachCallReturn(value.Common(), 0, func(_ *ssa.Function, returned ssa.Value) {
			if context.facadeUsesLiveCatalog(returned, seen) {
				found = true
			}
		})
		return found
	case *ssa.Extract:
		if call, ok := value.Tuple.(*ssa.Call); ok {
			found := false
			context.forEachCallReturn(call.Common(), value.Index, func(_ *ssa.Function, returned ssa.Value) {
				if context.facadeUsesLiveCatalog(returned, seen) {
					found = true
				}
			})
	...
	for _, dependency := range context.valueDependencies(value) {
		if context.facadeUsesLiveCatalog(dependency, seen) {
			return true
		}
	}
	return false
...
func (context *realCatalogSSAContext) discoverUsesLiveBundle(call *ssa.CallCommon) bool {
	for _, argument := range call.Args {
		...
		for _, callee := range context.resolvedCallees(call) {
			if (callee.Name() == "Discover" || callee.Name() == "DiscoverForDurableIntents") && context.analyzer.isCapabilitypackFunction(callee, callee.Name()) {
				return true
			}
		}
	}
...
func (context *realCatalogSSAContext) callReturnStrings(call *ssa.CallCommon, index int, seen map[ssa.Value]bool) map[string]struct{} {
	result := map[string]struct{}{}
	context.forEachCallReturn(call, index, func(_ *ssa.Function, returned ssa.Value) {
...
func (context *realCatalogSSAContext) forEachCallReturn(call *ssa.CallCommon, index int, analyze func(*ssa.Function, ssa.Value)) {
	for _, callee := range context.resolvedCallees(call) {
		...
		for _, returned := range functionReturnValues(callee, index) {
			analyze(callee, returned)
		}
		if hadPrevious {
