...
var realCatalogTestExceptions = map[string]realCatalogException{
	...
	"internal/claudesmoke/runner_test.go:TestEvidenceSchemaV4ProvesSnapshotThenExplicitActivation":                    {realCatalogGeneratedEvidence, "Protects Addy publication evidence ordering from selected snapshot inspection through explicit activation."},
	...
	"internal/cli/pack_test.go:TestPackListUsesOneCapturedWorkstationForInjectedTestSource":                           {realCatalogPublicContract, "Protects injected test-source selection without retaining the obsolete repository-discovery production route."},
