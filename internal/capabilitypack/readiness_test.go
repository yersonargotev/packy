...
func TestExecutableAcquisitionRequiresExplicitReviewedCapability(t *testing.T) {
	...
	facade := NewFacade(Catalog{}, WithExternalEffects(resolver, map[SurfaceCapabilityType]ExecutableAcquirer{SurfaceCapabilityExternalExecutableAcquisition: acquirer}, nil))
	...
	explicit := plain
	explicit.Resources = []Resource{{Bindings: []Binding{{Surface: SurfaceCodex, Capabilities: []SurfaceCapability{{Type: SurfaceCapabilityExternalExecutableAcquisition, ExternalExecutableAcquisition: &ExternalExecutableAcquisitionCapability{Tool: "engram"}}}}}}}
	explicitResolutions, err := facade.resolveExecutables(context.Background(), explicit, SurfaceCodex, true)
	if err != nil {
		t.Fatal(err)
	}
	if acquirer.calls != 1 || len(explicitResolutions) != 1 || !explicitResolutions[0].AcquisitionSupported || explicitResolutions[0].Capability != SurfaceCapabilityExternalExecutableAcquisition || explicitResolutions[0].AcquisitionCommand != "brew" {
