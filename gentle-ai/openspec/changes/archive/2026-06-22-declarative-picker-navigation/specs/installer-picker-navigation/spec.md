# Delta Spec: Declarative Picker Navigation (Computed Flow Slice)

## Change

`declarative-picker-navigation`

## Relation to Existing Spec

This is a behavior-preservation delta for the installer TUI picker-chain navigation. It introduces NO new or modified product capability: the externally observable navigation behavior is invariant. The requirements below capture the navigation contract that MUST continue to hold after the internal refactor that replaced the triplicated imperative navigation logic with a single computed flow slice (`pickerFlowSlice` + `pickerNextScreen` / `pickerPreviousScreen` / `applyPickerEntry`). All requirements are enforced by the existing TUI test suite.
