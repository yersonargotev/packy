...
Artifact store: hybrid (mirrored to openspec/changes/memory-conflict-surfacing/verify-report.md)
Mode: Strict TDD
...
Status: done — 0 CRITICAL, 0 WARNING, 1 SUGGESTION (deferred to Phase 2). Recommendation: **READY_TO_ARCHIVE**.
...
### Spot-check of unchanged surfaces (still passing)

| Surface | Status | Evidence |
|---------|--------|----------|
| REQ-002 (search annotations) | PASS — unchanged | `internal/mcp/mcp.go:824-842` annotation switch intact |
| REQ-003 (mem_judge tool) | PASS — unchanged | `internal/mcp/mcp.go:665-717,1496-1557` |
| REQ-004 (multi-actor schema) | PASS — unchanged | `internal/store/store.go:792-810` |
| REQ-005 (provenance) | PASS — unchanged | `internal/store/relations.go:358-381` |
| REQ-006 (decay schema) | PASS — unchanged | `internal/store/store.go:182-195,2000-2011` |
| REQ-007 (backwards compat) | PASS — unchanged | `internal/mcp/mcp_conflict_loop_test.go:502-624` |
| REQ-008 (migration safety) | PASS — unchanged | `internal/store/store.go:771-823` |
| REQ-009 (local-only relations) | PASS — unchanged | `internal/store/store.go:309-327` |
| REQ-010 (orphaning) | PASS — unchanged | `internal/store/store.go:2406-2419`, `internal/store/relations.go:420-422` |
| 40 tasks marked done | CONFIRMED | apply-progress (#2631) Phases A–H + Addendum all `[DONE]` |
...
| File | Action | Notes |
|------|--------|-------|
| `internal/mcp/mcp.go` | Modified | Added `BM25Floor *float64` and `Limit *int` to `MCPConfig` (lines 34-50). Wired both through `handleSave` (lines 965-974). |
| `internal/mcp/mcp_test.go` | Appended | `TestHandleSave_MCPConfig_OverridesDefaults` (lines 3791-3854). RED→GREEN cycle documented. |
| `internal/store/relations.go` | Modified | `CandidateOptions.BM25Floor` changed to `*float64` (line 65). Default detection updated to pointer-nil check (lines 162-167). |
| `internal/store/relations_test.go` | Modified + Appended | All 6 BM25Floor caller sites migrated to `ptrFloat64(...)`. Added `TestFindCandidates_ExplicitZeroFloor` (lines 784-828) and `ptrFloat64` helper (lines 830-831). |
